/* Browser root-cause probe: execute the real Get_scene while bypassing input/SceneManager only. */
...
(async () => {
  ...
  assert.deepEqual(initial.arrayFromSet, ['alpha', 'beta'],
    'browser-native Array.from must survive phina.js loading');
  ...
  // Reproduce the state-changing part of the title start handler without invoking SceneManager.
  await page.evaluate(() => {
    toho_start_run();
    player = new Player();
    set_cookies();
  ...
  assert.equal(afterStart.tracking, true, 'fresh Player must be wrapped by encyclopedia tracking');
  ...
  await page.evaluate(() => {
    ...
    // Production Get_scene.init performs random drop selection, player.get_item, then set_cookies.
  ...
  // Abandon ultimately calls delete_cookies(); it must remove run progress but not encyclopedia meta.
  ...
  // Fresh load must reconstruct the same encyclopedia from localStorage.
  await page.reload({ waitUntil: 'domcontentloaded' });
  await ready();
  ...
  console.log('PASS: real browser Get_scene persists encyclopedia through delete_cookies and reload');
  await browser.close();
})().catch(error => {
